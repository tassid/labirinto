...
console.log(maze); // Output the generated maze
...
function weightedRandom(weights) {
  ...
  // Fallback to 'sand' if there are issues with the weights
  return 'sand';
}
...
function weightedRandom(weights) {
  ...
  function create() {
    ...
    const maze = generateRandomMaze(50, 50, 0.5, 0.5);
  ...
  const width = 50;
  const height = 50;
  const complexity = 0.5; // Adjust complexity and density as needed
  const density = 0.5;
  const maze = generateRandomMaze(width, height, complexity, density);
